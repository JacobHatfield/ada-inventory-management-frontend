<template>
  <div class="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      <!-- Search Input -->
      <div>
        <label for="search" class="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">
          Search
        </label>
        <div class="relative">
      ...
      <!-- Category Filter -->
      <div>
        <label for="category" class="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">
          Category
        </label>
        <select
          id="category"
      ...
      <!-- Stock Status Filter -->
      <div>
        <label for="stockStatus" class="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">
          Stock Status
        </label>
        <select
          id="stockStatus"
      ...
      <!-- Sort -->
      <div>
        <label for="sort" class="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">
          Sort By
        </label>
        <select
          id="sort"
...
watch(
  ...
  (newVal) => { localCategoryId.value = newVal; }
);
watch(
  () => props.initialStockStatus,
  (newVal) => { localStockStatus.value = newVal; }
);
watch(
  () => [props.initialSortBy, props.initialSortOrder],
  () => {
