...
export class ApiError extends Error {
  ...
  constructor(message: string, status: number | null, options?: { code?: string; details?: unknown; raw?: unknown }) {
    super(message)
    this.name = 'ApiError'
    this.status = status
...
function normalizeErrorPayload(payload: unknown): { message: string; code?: string; details?: unknown } {
  if (!isRecord(payload)) {
...
export async function apiRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const { method = 'GET', token, body, headers = {}, timeoutMs = DEFAULT_TIMEOUT_MS, signal, skipAuth = false } = options
  ...
  try {
    ...
    const apiError = new ApiError(isAbortError ? 'Request timed out or was aborted' : 'Network error', null, {
      raw: error,
    })
